    ...
    try {
        ...
        res.status(201).json({
            ...
            candidate: {
                ...
                // Don't send encryptedVotes or vote count in response
            }
        });
    } catch (error) {
...
const getCandidate = async (req, res, next) => {
    try {
        ...
        const candidate = await CandidateModel.findById(id)
            .select('-encryptedVotes'); // Exclude encrypted votes from response
...
const voteCandidate = async (req, res, next) => {
    try {
        ...
        // Create the vote object with proper voter ID
        const vote = {
            voter: new mongoose.Types.ObjectId(voterId),
            encryptedValue: {
                c1: encryptedValue.c1,
                c2: encryptedValue.c2
            }
        };

        // Store the encrypted vote
        candidate.encryptedVotes.push(vote);
...
const getCandidatesWithEncryptedVotes = async (req, res, next) => {
    try {
        const { electionId } = req.params;
        const election = await ElectionModel.findById(electionId);
        ...
        // Only return results after result time
        if (now <= resultTime) {
            return next(new HttpError('Results are not available yet', 403));
        }

        // Get candidates with encrypted votes
        const candidates = await CandidateModel.find({ election: electionId })
            .select('+encryptedVotes');

        // Process vote data
        const processedCandidates = candidates.map(candidate => {
            return {
                ...candidate.toObject(),
                encryptedVotes: candidate.encryptedVotes || []
            };
        });

        console.log('Processed candidates with votes:', processedCandidates);
            
        res.status(200).json(processedCandidates);
    } catch (error) {
        console.error('Error fetching candidates with votes:', error);
        return next(new HttpError('Failed to fetch candidates with votes', 500));
    }
};
